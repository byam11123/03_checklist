...
function updatePassword(e) {
  try {
    ...
    var lastRow = loginSheet.getLastRow();
    var sheetData = loginSheet.getRange(2, 1, lastRow - 1, 3).getValues();
    
    // Find user and verify current password
    for (var i = 0; i < sheetData.length; i++) {
      if (sheetData[i][0] === name && sheetData[i][1].toString() === currentPassword) {
        // Update password
        loginSheet.getRange(i + 2, 2).setValue(newPassword);
